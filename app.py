...
with st.sidebar:
    ...
    st_lottie.st_lottie(lottie_animation, height=200,
                        width=300, key="lottie_animation")
    st.markdown('</div>', unsafe_allow_html=True)
    st.markdown("<h2 style='color: #007bff;' align='center'>Explore the App!</h2>",
                unsafe_allow_html=True)
    st.markdown(
        "**About the Model:** This MNIST digit recognizer uses a convolutional neural network trained on handwritten digits.")
...
upload_or_draw = st.selectbox("Choose Input Method:", [
...
if upload_or_draw == "Upload an Image":
    ...
    image_file = st.file_uploader("Upload an image (jpg, png, jpeg, etc.)", type=[
                                  "jpg", "jpeg", "png"], key="file_uploader")
